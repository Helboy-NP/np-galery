...
// FUNGSI MEMUAT DATA DARI PRICELIST.JSON
async function loadPriceListData() {
    try {
        const response = await fetch('pricelist.json');
        if (!response.ok) throw new Error('File pricelist.json tidak dapat dimuat');
        rawPriceListData = await response.json();
        ...
        console.warn('Memuat pricelist.json gagal atau berjalan di protokol file:/// :', error);
        rawPriceListData = [];
        initBrandDropdown();
        initReportBrandDropdown();
        filterPriceList();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Panggil pemuatan data JSON master price list & diagnosis
    loadPriceListData();
    loadDiagnosisData();
...
/* ========================================================== */
/* FITUR BARU 1: SCANNER KAMERA HP SUNGGUHAN (HTML5-QRCODE)   */
/* ========================================================== */
...
/* ========================================================== */
/* FITUR BARU 2: POP-UP CHAT WHATSAPP LANGSUNG                */
/* ========================================================== */
...
function openDiagnosisModal(query) {
    ...
    // Cari merek yang cocok dari rawDiagnosisData
...
window.addNewProduct = function() {
    ...
    rawPriceListData.unshift({ id: newId, brand: detectedBrand, model: detectedModel, jkt: jktInput || '--', sgc: sgcInput || '--' });
    selectedPriceListModelIds.add(newId);

    initBrandDropdown(); initReportBrandDropdown(); filterPriceList(); renderLaporanKeuangan();
    document.getElementById('add-input-name').value = ''; document.getElementById('add-input-jkt').value = ''; document.getElementById('add-input-sgc').value = '';
...
window.simpanStokBaru = function() {
    ...
    daftarStokMasuk.unshift({
        id: 'stok-' + Date.now(), produk, kondisi: document.getElementById('stok-kondisi').value,
        kelengkapan: document.getElementById('stok-kelengkapan').value, imei,
        qty: document.getElementById('stok-qty').value || '1', hargaModal: document.getElementById('stok-harga').value || '0', hargaJual: '', pembeli: '', tanggal
    });
    saveStokToStorage(); renderDaftarStokMasuk(); updateDashboardStats(); renderLaporanKeuangan();
    showToast('Tersimpan', 'Stok baru masuk.');
    document.getElementById('stok-produk-input').value = ''; document.getElementById('stok-imei').value = '';
};
...
window.updateHargaJualLive = function(val) {
    ...
    if (item) { item.hargaJual = val; saveStokToStorage(); }
};

window.updatePembeliLive = function(val) {
    const item = daftarStokMasuk.find(s => s.id === activeDetailStokId);
    if (item) { item.pembeli = val; saveStokToStorage(); }
};
...
window.jualStokItem = function(id) {
    const item = daftarStokMasuk.find(s => s.id === id);
    if (item) {
        showCustomConfirm("Penjualan", `Jual unit ${item.produk}?`, () => {
            ...
            if (profit > 0) {
                daftarKasPribadi.unshift({ id: 'kas-' + Date.now(), keterangan: `Laba Jual: ${item.produk}`, kategori: 'masuk', nominal: profit, tanggal: tgl });
                saveKasToStorage();
            }
        ...
        <div class="elegant-invoice-card">
            ...
            <div class="invoice-footer-signatures">
                ...
                <div class="invoice-sign-column-right">
                    <span class="invoice-sign-header-label">Pembeli / Penerima,</span>
...
window.hapusStokItem = function(id) {
    showCustomConfirm("Hapus Stok", "Hapus unit ini?", () => {
        daftarStokMasuk = daftarStokMasuk.filter(s => s.id !== id);
        saveStokToStorage(); renderDaftarStokMasuk(); updateDashboardStats(); renderLaporanKeuangan();
    });
};
...
window.hapusRiwayatTerjual = function(id) {
    showCustomConfirm("Hapus Riwayat", "Hapus riwayat penjualan?", () => {
...
window.toggleModalStatus = function(id) {
    ...
    if (item) { item.modalStatus = item.modalStatus === 'sudah' ? 'belum' : 'sudah'; saveTerjualToStorage(); renderDaftarModal(); }
};
...
window.tambahKasPribadi = function() {
    ...
    daftarKasPribadi.unshift({ id: 'kas-' + Date.now(), keterangan: ket, kategori: document.getElementById('kas-kategori').value, nominal: nom, tanggal: tgl });
    saveKasToStorage();
    document.getElementById('kas-keterangan').value = ''; document.getElementById('kas-nominal').value = '';
    renderManajemenKas(); updatePribadiStats(); renderLaporanKeuangan();
    showToast('Berhasil', 'Kas ditambahkan.');
};
window.hapusKasPribadi = function(id) {
    showCustomConfirm("Hapus Kas", "Hapus catatan kas ini?", () => {
        daftarKasPribadi = daftarKasPribadi.filter(k => k.id !== id);
        saveKasToStorage(); renderManajemenKas(); updatePribadiStats(); renderLaporanKeuangan();
    });
};
...
window.broadcastStokWA = function() {
    ...
    if (navigator.clipboard) {
        navigator.clipboard.writeText(text).then(() => {
            showToast('Tersalin!', 'Teks broadcast disalin ke clipboard.');
            setTimeout(() => {
                window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent(text)}`, '_blank');
            }, 600);
        }).catch(() => {
            ...
            return `
                <div class="laporan-detail-item" style="padding: 6px 10px;">
                    <label class="checkbox-model-item">
                        ...
                        <div class="detail-left" style="flex:1;"><span class="detail-title">${p.brand} - ${p.model}</span><span class="detail-sub">JKT: ${formatDisplayPrice(p.jkt)} | SGC: ${formatDisplayPrice(p.sgc)}</span></div>
                    </label>
                </div>
            `;
        }).join('');
    ...
    if (activeReportCategory === 'kas') {
        ...
        tableRowsHtml = d.filteredPLSelected.length === 0 ? `<tr><td colspan="4" align="center" style="color:#64748B;">Tidak ada model terpilih.</td></tr>` : d.filteredPLSelected.map((p, i) => `
            <tr>
                ...
                <td>Harga Jkt: ${formatDisplayPrice(p.jkt)}</td>
                <td align="right" style="font-weight:700; color:#0077B6; font-family:var(--font-mono);">SGC: ${formatDisplayPrice(p.sgc)}</td>
            </tr>
        `).join('');
    }
...
function generatePlainTextReport() {
    ...
    if (currentCatKey === 'kas') {
        ...
        d.filteredPLSelected.forEach((p, i) => {
            text += `${i+1}. [${p.brand}] ${p.model} | JKT: ${p.jkt} | SGC: ${p.sgc}\n`;
        });
    }
    return text;
}
...
function generateCSVReport() {
    ...
    if (currentCatKey === 'kas') {
        ...
        d.filteredPLSelected.forEach((p, i) => {
            csv += `"${i+1}","PRICELIST","${p.brand} ${p.model}","JKT: ${p.jkt}","SGC: ${p.sgc}"\n`;
        });
    }
    return csv;
}
